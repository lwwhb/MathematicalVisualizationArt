...
half3 calculateLighting(float3 position, float3 lightDirI, float3 normal, float3 viewDirI, half3 lightColor, half lightIntensity, half indirectLightIntensity, half attenuation, Material material, half3 bgColor)
{
    ...
    float3 reflectDir = normalize(reflect(-lightDirI, normal));     //反射向量
    ...
    float RoV = saturate(dot(reflectDir, viewDirI));
    ...
    half3 indirectColor = LightingIndirect(position, normal, reflectDir, NoV, indirectLightIntensity, f0, diffuseColor, bgColor);
