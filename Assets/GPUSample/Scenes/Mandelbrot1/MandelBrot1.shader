Shader "MathematicalVisualizationArt/MandelBrot1"
{
    Properties
    {
    }
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            half3 PixelColor(float2 uv)
            {
                ...
                for(k = 0.0; k < 256.0; k++)
                {
                    z = float2( z.x*z.x - z.y*z.y, 2.0*z.x*z.y ) + (uv - 0.5) * float2(w, h)/B; // z = z² + c
                    if( dot(z,z) > 4 )
                        break;
                }
                c.xyz = half3((log(k)*147)/255.0,(log(k)*47)/255.0, (128.0 -log(k)*23.0)/255.0);
                //
                return c;
            }
