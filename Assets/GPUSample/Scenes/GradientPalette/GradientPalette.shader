Shader "MathematicalVisualizationArt/GradientPalette"
{
    Properties
    {
    }
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            half3 PixelColor(float2 uv)
            {
                ...
                float3 s = float3(0.0f, -2.0 * acos(-1.0) / 3.0, 2.0f * acos(-1.0) / 3.0);
                c = pow(cos(atan2(uv.yyy-0.5f,uv.xxx-0.5f)/2.0f + s + time), 2.0f);
                //
                return c;
            }
