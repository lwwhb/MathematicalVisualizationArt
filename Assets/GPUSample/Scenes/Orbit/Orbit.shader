Shader "MathematicalVisualizationArt/Orbit"
{
    Properties
    {
    }
        ...
        Pass
        {
            ...
            half3 PixelColor(float2 uv)
            {
                ...
                for(uint i=0; i < 400; i++) {
                    ...
                    half o2 = saturate(floor((s[i%10] - (i * 0.001) - 0.003)/length(float2(uv.x - 0.5f, (uv.y - 0.5f) * (h / w)))))*a;
                    orbit = orbit + (o2 - o1);
                } 
                float edge = 0.35;
                float smooth = 0.5;
                ...
                c = saturate(c + pow((uv.x + uv.y) * 0.5, 4) - orbit);
                //---
                return c;
            }
