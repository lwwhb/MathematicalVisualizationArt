Shader "MathematicalVisualizationArt/FaceTheDarkLord_Revision"
{
    Properties
    {
    }
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            half3 PixelColor(float2 uv)
            {
                ...
                half2 uv1 = half2(uv.x-0.5f, (uv.y-0.5f) );half2 p_c = half2(atan2(uv1.x,uv1.y)/3.1415926,length(uv1));
                half wd_a = p_c.x <= 0.74f && p_c.x >= 0.35f ? 1 : 0;
                half4 w_t = half4(uv,0,0);
                {
                    w_t.y = dot(normalize(uv1), float2(0.,1.));
                    w_t.x *= lerp(1,0,uv1.x) + 1.7;
                    w_t.x -= time * 0.1;
                    w_t.x *= 5.;w_t.y *= 1.;
                    float x_id = floor(w_t.x);float y_id = floor(w_t.y);
                ...
                half2 uv2 = half2((uv.y-0.5f), (uv.x-0.5f));
                uv2.y += 0.1;
                half2 p_c2 = half2(atan2(uv2.x,uv2.y)/3.1415926,length(uv2));
                half r_a = p_c2.x <= 0.74f && p_c2.x >= 0.35f ? 1 : 0;
                half4 r_t = half4(uv,0,0);
                {
                    r_t.x = dot(normalize(uv2), float2(0.,1.));
                    r_t.y *= lerp(1,0,uv2.x) + 1.7;
                    r_t.y -= time * 0.1;
                    r_t.y *= 5.;
                    r_t.x *= 1.;
                    float x_id = floor(r_t.x);
                    float y_id = floor(r_t.y);
                    r_t = float4(frac(r_t.xy),x_id,y_id);
                }
                half r = 0;
                ...
                c = (step(1.,r * fmod(abs(r_t.a),2.0)) * r_a) * float3(uv2.y,0,uv2.x);
                c += (step(1., wd * fmod(abs(w_t.z),2.0)) * wd_a) * float3(uv1.x,0,uv1.y);
                return c;
            }
